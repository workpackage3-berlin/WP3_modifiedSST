...
expName = 'mSST_v18_german_LSL'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Juliette\\OneDrive\\Documents\\PhD\\Psychopy\\modifiedStopSignalTask\\mSST_v18_german_LSL.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    late_key_resp_training = keyboard.Keyboard()
    ...
    go_rt_training = []
    correct_training = []
    missed_squares_training = 0
    correct_stop_training = []
    n_ST_training = 0
    ...
    # --- Initialize components for Routine "stats_training" ---
    
    # --- Initialize components for Routine "ssd_screen" ---
    
    # --- Initialize components for Routine "stats_text" ---
    text_stats = visual.TextStim(win=win, name='text_stats',
        text='',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    missed_squares = 0
    correct_stop = []
    n_stops_this_block = 0
    n_trial_this_block = 0
    ...
    # --- Initialize components for Routine "stats_main" ---
    ...
    # --- Initialize components for Routine "stats_main_screen" ---
    stats_text_main = visual.TextStim(win=win, name='stats_text_main',
        text='',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    break_stats_screen = keyboard.Keyboard()
    
    # --- Initialize components for Routine "reset_variables" ---
    
    # --- Initialize components for Routine "pulses_between_blocks" ---
    pulse_text = visual.TextStim(win=win, name='pulse_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    key_resp_intermediate_pulses = keyboard.Keyboard()
    ...
    percentage_GC = 0.17
    percentage_ST = 0.17
    # too many GF trials, people only used proactive inhibition
    """
    # new proportions (25.07.2024)
    percentage_GT = 0.5
    percentage_GF = 0.17
    percentage_GC = 0.17
    percentage_ST = 0.17
    ...
    for thisTrial_loop_training in trial_loop_training:
        ...
        continueRoutine = continueRoutine and not (early_press_resp_training.corr)
        ...
        continueRoutine = True
        # update component parameters for each repeat
        late_key_resp_training.keys = []
        late_key_resp_training.rt = []
        _late_key_resp_training_allKeys = []
        # keep track of which components have finished
        blank_trainingComponents = [no_shape_2, late_key_resp_training]
        ...
        while continueRoutine:
            ...
            # *late_key_resp_training* updates
            waitOnFlip = False
            
            # if late_key_resp_training is starting this frame...
            if late_key_resp_training.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                late_key_resp_training.frameNStart = frameN  # exact frame index
                late_key_resp_training.tStart = t  # local t and not account for scr refresh
                late_key_resp_training.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(late_key_resp_training, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'late_key_resp_training.started')
                # update status
                late_key_resp_training.status = STARTED
                # AllowedKeys looks like a variable named `right_or_left`
                if not type(right_or_left) in [list, tuple, np.ndarray]:
                    if not isinstance(right_or_left, str):
                        logging.error('AllowedKeys variable `right_or_left` is not string- or list-like.')
                ...
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(late_key_resp_training.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(late_key_resp_training.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if late_key_resp_training.status == STARTED and not waitOnFlip:
                theseKeys = late_key_resp_training.getKeys(keyList=list(right_or_left), ignoreKeys=["escape"], waitRelease=False)
                _late_key_resp_training_allKeys.extend(theseKeys)
                if len(_late_key_resp_training_allKeys):
                    late_key_resp_training.keys = _late_key_resp_training_allKeys[-1].name  # just the last key pressed
                    late_key_resp_training.rt = _late_key_resp_training_allKeys[-1].rt
                    late_key_resp_training.duration = _late_key_resp_training_allKeys[-1].duration
                    # was this correct?
                    if (late_key_resp_training.keys == str(key_resp)) or (late_key_resp_training.keys == key_resp):
                        late_key_resp_training.corr = 1
                    else:
                        late_key_resp_training.corr = 0
        ...
        for thisComponent in blank_trainingComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # check responses
        if late_key_resp_training.keys in ['', [], None]:  # No response was made
            late_key_resp_training.keys = None
            # was no response the correct answer?!
            if str(key_resp).lower() == 'none':
               late_key_resp_training.corr = 1;  # correct non-response
            else:
               late_key_resp_training.corr = 0;  # failed to respond (incorrectly)
        # store data for trial_loop_training (TrialHandler)
        trial_loop_training.addData('late_key_resp_training.keys',late_key_resp_training.keys)
        trial_loop_training.addData('late_key_resp_training.corr', late_key_resp_training.corr)
        if late_key_resp_training.keys != None:  # we had a response
            trial_loop_training.addData('late_key_resp_training.rt', late_key_resp_training.rt)
            trial_loop_training.addData('late_key_resp_training.duration', late_key_resp_training.duration)
        # the Routine "blank_training" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        if key_resp_training.corr and not early_press_resp_training.corr and not late_key_resp_training.corr:
            if trial_type in ['go_trial', 'go_fast_trial', 'go_continue_trial']:
                ...
                n_ST_training += 1
                correct_stop_training.append(key_resp_training.corr)
                thisExp.addData('total_score_training',total_score_training)
                msg = 'richtig'
        elif not early_press_resp_training.corr:
            if trial_type in ['go_trial', 'go_fast_trial', 'go_continue_trial']:
                total_score_training -= 1
                missed_squares_training += 1
                thisExp.addData('total_score_training',total_score_training)
                msg = 'falsch'
            else:
                total_score_training -= 3
        ...
        if trial_type == 'go_trial':
            if key_resp_training.corr:
                go_rt_training.append(key_resp_training.rt)
                
        correct_training.append(key_resp_training.corr)
    ...
    # --- Prepare to start Routine "stats_training" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('stats_training.started', globalClock.getTime())
    # keep track of which components have finished
    stats_trainingComponents = []
    for thisComponent in stats_trainingComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "stats_training" ---
    for thisComponent in stats_trainingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('stats_training.stopped', globalClock.getTime())
    # Run 'End Routine' code from code_stats_training
    import numpy
    
    go_rt_training_array = numpy.array(go_rt_training)
    mean_go_rt_training = go_rt_training_array.mean()
    mean_go_rt_training_ms = str(round(mean_go_rt_training * 1000))
    
    percent_correct_array_training = numpy.array(correct_training)
    percent_correct_training = str(round((sum(percent_correct_array_training) / n_training_trials)*100))
    
    missed_squares_training = str(missed_squares_training)
    
    percent_correct_stop_array_training = numpy.array(correct_stop_training)
    percent_correct_stop_training = str(round((sum(percent_correct_stop_array_training) /  n_ST_training)*100))
    # the Routine "stats_training" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    # --- Prepare to start Routine "stats_text" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('stats_text.started', globalClock.getTime())
    text_stats.setText("mittlere Reaktionszeit: " + mean_go_rt_training_ms + "ms \n\n Korrekt: " + percent_correct_training + "% \n\n Anzahl der Quadrate ohne graues Dreieck, die ausgelassen wurden: " + missed_squares_training + " (sollte 0 sein) \n\n p = " +  percent_correct_stop_training + "%")
    skip_stats.keys = []
    skip_stats.rt = []
    _skip_stats_allKeys = []
    # keep track of which components have finished
    stats_textComponents = [text_stats, skip_stats]
    for thisComponent in stats_textComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if text_stats is starting this frame...
        if text_stats.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            text_stats.frameNStart = frameN  # exact frame index
            text_stats.tStart = t  # local t and not account for scr refresh
            text_stats.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_stats, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_stats.started')
            # update status
            text_stats.status = STARTED
            text_stats.setAutoDraw(True)
        
        # if text_stats is active this frame...
        if text_stats.status == STARTED:
            # update params
            pass
        ...
        # if skip_stats is starting this frame...
        if skip_stats.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            skip_stats.frameNStart = frameN  # exact frame index
            skip_stats.tStart = t  # local t and not account for scr refresh
            skip_stats.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(skip_stats, 'tStartRefresh')  # time at next scr refresh
            # update status
            skip_stats.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(skip_stats.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(skip_stats.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if skip_stats.status == STARTED and not waitOnFlip:
            theseKeys = skip_stats.getKeys(keyList=['p'], ignoreKeys=["escape"], waitRelease=False)
            _skip_stats_allKeys.extend(theseKeys)
            if len(_skip_stats_allKeys):
                skip_stats.keys = _skip_stats_allKeys[-1].name  # just the last key pressed
                skip_stats.rt = _skip_stats_allKeys[-1].rt
                skip_stats.duration = _skip_stats_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # --- Ending Routine "stats_text" ---
    for thisComponent in stats_textComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('stats_text.stopped', globalClock.getTime())
    # check responses
    if skip_stats.keys in ['', [], None]:  # No response was made
        skip_stats.keys = None
    thisExp.addData('skip_stats.keys',skip_stats.keys)
    if skip_stats.keys != None:  # we had a response
        thisExp.addData('skip_stats.rt', skip_stats.rt)
        thisExp.addData('skip_stats.duration', skip_stats.duration)
    thisExp.nextEntry()
    # the Routine "stats_text" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisBlock in blocks:
        ...
        for thisTrial_loop in trial_loop:
            ...
            # skip this Routine if its 'Skip if' condition is True
            continueRoutine = continueRoutine and not (early_press_resp.corr)
            ...
            while continueRoutine:
                ...
                if late_key_resp1.status == STARTED and not waitOnFlip:
                    theseKeys = late_key_resp1.getKeys(keyList=list(right_or_left), ignoreKeys=["escape"], waitRelease=False)
                    ...
                    if len(_late_key_resp1_allKeys):
                        ...
                        # was this correct?
                        if (late_key_resp1.keys == str(key_resp)) or (late_key_resp1.keys == key_resp):
                            late_key_resp1.corr = 1
                        else:
                            late_key_resp1.corr = 0
            ...
            if late_key_resp1.keys in ['', [], None]:  # No response was made
                ...
                # was no response the correct answer?!
                if str(key_resp).lower() == 'none':
                   late_key_resp1.corr = 1;  # correct non-response
                else:
                   late_key_resp1.corr = 0;  # failed to respond (incorrectly)
            # store data for trial_loop (TrialHandler)
            trial_loop.addData('late_key_resp1.keys',late_key_resp1.keys)
            trial_loop.addData('late_key_resp1.corr', late_key_resp1.corr)
            ...
            if key_resp_experiment.corr and not early_press_resp.corr and not late_key_resp1.corr:
                if trial_type in ['go_trial', 'go_fast_trial', 'go_continue_trial']:
                    total_score += 1
                    ...
                    total_score += 3
                    n_stops_this_block += 1
                    correct_stop.append(key_resp_experiment.corr)
        ...
        # --- Prepare to start Routine "stats_main" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('stats_main.started', globalClock.getTime())
        # keep track of which components have finished
        stats_mainComponents = []
        for thisComponent in stats_mainComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "stats_main" ---
        for thisComponent in stats_mainComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('stats_main.stopped', globalClock.getTime())
        # Run 'End Routine' code from code_stats_main_task
        import numpy
        
        go_rt_array = numpy.array(go_rt)
        mean_go_rt = go_rt_array.mean()
        mean_go_rt_ms = str(round(mean_go_rt * 1000))
        print(mean_go_rt_ms)
        
        percent_correct_array = numpy.array(correct)
        percent_correct = str(round((sum(percent_correct_array) / n_trials)*100))
        
        missed_squares = str(missed_squares)
        
        percent_correct_stop_array = numpy.array(correct_stop)
        percent_correct_stop = str(round((sum(percent_correct_stop_array) /  n_stops_this_block)*100))
        
        n_trial_this_block = str(n_trial_this_block)
        # the Routine "stats_main" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        # --- Prepare to start Routine "stats_main_screen" ---
        continueRoutine = True
        # update component parameters for each repeat
        stats_text_main.setText("mittlere Reaktionszeit: " + mean_go_rt_ms + "ms \n\n Korrekt: " + percent_correct + "% \n\n Anzahl der Quadrate ohne graues Dreieck, die ausgelassen wurden: " + missed_squares + " (sollte 0 sein) \n\n p = " +  percent_correct_stop + "% \n\n n trials = " + n_trial_this_block + " trials")
        break_stats_screen.keys = []
        break_stats_screen.rt = []
        _break_stats_screen_allKeys = []
        # keep track of which components have finished
        stats_main_screenComponents = [stats_text_main, break_stats_screen]
        for thisComponent in stats_main_screenComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # *stats_text_main* updates
            
            # if stats_text_main is starting this frame...
            if stats_text_main.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                stats_text_main.frameNStart = frameN  # exact frame index
                stats_text_main.tStart = t  # local t and not account for scr refresh
                stats_text_main.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(stats_text_main, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'stats_text_main.started')
                # update status
                stats_text_main.status = STARTED
                stats_text_main.setAutoDraw(True)
            
            # if stats_text_main is active this frame...
            if stats_text_main.status == STARTED:
                # update params
                pass
            
            # *break_stats_screen* updates
            waitOnFlip = False
            
            # if break_stats_screen is starting this frame...
            if break_stats_screen.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                break_stats_screen.frameNStart = frameN  # exact frame index
                break_stats_screen.tStart = t  # local t and not account for scr refresh
                break_stats_screen.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(break_stats_screen, 'tStartRefresh')  # time at next scr refresh
                # update status
                break_stats_screen.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(break_stats_screen.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(break_stats_screen.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if break_stats_screen.status == STARTED and not waitOnFlip:
                theseKeys = break_stats_screen.getKeys(keyList=['p'], ignoreKeys=["escape"], waitRelease=False)
                _break_stats_screen_allKeys.extend(theseKeys)
                if len(_break_stats_screen_allKeys):
                    break_stats_screen.keys = _break_stats_screen_allKeys[-1].name  # just the last key pressed
                    break_stats_screen.rt = _break_stats_screen_allKeys[-1].rt
                    break_stats_screen.duration = _break_stats_screen_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # --- Ending Routine "stats_main_screen" ---
        for thisComponent in stats_main_screenComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # check responses
        if break_stats_screen.keys in ['', [], None]:  # No response was made
            break_stats_screen.keys = None
        blocks.addData('break_stats_screen.keys',break_stats_screen.keys)
        if break_stats_screen.keys != None:  # we had a response
            blocks.addData('break_stats_screen.rt', break_stats_screen.rt)
            blocks.addData('break_stats_screen.duration', break_stats_screen.duration)
        # the Routine "stats_main_screen" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "reset_variables" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('reset_variables.started', globalClock.getTime())
        # keep track of which components have finished
        reset_variablesComponents = []
        for thisComponent in reset_variablesComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "reset_variables" ---
        for thisComponent in reset_variablesComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('reset_variables.stopped', globalClock.getTime())
        # Run 'End Routine' code from reset_for_next_block
        go_rt = []
        correct = []
        missed_squares = 0
        correct_stop = []
        ...
        # the Routine "reset_variables" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        for thisBreak_screen_loop in break_screen_loop:
            ...
            # --- Prepare to start Routine "pulses_between_blocks" ---
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('pulses_between_blocks.started', globalClock.getTime())
            key_resp_intermediate_pulses.keys = []
            key_resp_intermediate_pulses.rt = []
            _key_resp_intermediate_pulses_allKeys = []
            # keep track of which components have finished
            pulses_between_blocksComponents = [pulse_text, key_resp_intermediate_pulses]
            for thisComponent in pulses_between_blocksComponents:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "pulses_between_blocks" ---
            routineForceEnded = not continueRoutine
            while continueRoutine:
                # get current time
                t = routineTimer.getTime()
                ...
                # if pulse_text is starting this frame...
                if pulse_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    pulse_text.frameNStart = frameN  # exact frame index
                    pulse_text.tStart = t  # local t and not account for scr refresh
                    pulse_text.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(pulse_text, 'tStartRefresh')  # time at next scr refresh
                    # update status
                    pulse_text.status = STARTED
                    pulse_text.setAutoDraw(True)
                
                # if pulse_text is active this frame...
                if pulse_text.status == STARTED:
                    # update params
                    pass
                
                # *key_resp_intermediate_pulses* updates
                waitOnFlip = False
                
                # if key_resp_intermediate_pulses is starting this frame...
                if key_resp_intermediate_pulses.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    key_resp_intermediate_pulses.frameNStart = frameN  # exact frame index
                    key_resp_intermediate_pulses.tStart = t  # local t and not account for scr refresh
                    key_resp_intermediate_pulses.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(key_resp_intermediate_pulses, 'tStartRefresh')  # time at next scr refresh
                    # update status
                    key_resp_intermediate_pulses.status = STARTED
                    # keyboard checking is just starting
                    waitOnFlip = True
                    win.callOnFlip(key_resp_intermediate_pulses.clock.reset)  # t=0 on next screen flip
                    win.callOnFlip(key_resp_intermediate_pulses.clearEvents, eventType='keyboard')  # clear events on next screen flip
                if key_resp_intermediate_pulses.status == STARTED and not waitOnFlip:
                    theseKeys = key_resp_intermediate_pulses.getKeys(keyList=['p'], ignoreKeys=["escape"], waitRelease=False)
                    _key_resp_intermediate_pulses_allKeys.extend(theseKeys)
                    if len(_key_resp_intermediate_pulses_allKeys):
                        key_resp_intermediate_pulses.keys = _key_resp_intermediate_pulses_allKeys[-1].name  # just the last key pressed
                        key_resp_intermediate_pulses.rt = _key_resp_intermediate_pulses_allKeys[-1].rt
                        key_resp_intermediate_pulses.duration = _key_resp_intermediate_pulses_allKeys[-1].duration
                        # a response ends the routine
                        continueRoutine = False
            ...
            # --- Ending Routine "pulses_between_blocks" ---
            for thisComponent in pulses_between_blocksComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('pulses_between_blocks.stopped', globalClock.getTime())
            # the Routine "pulses_between_blocks" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
