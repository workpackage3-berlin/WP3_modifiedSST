...
expName = 'mSST_v14_german_LSL'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Juliette\\OneDrive\\Documents\\PhD\\Psychopy\\modifiedStopSignalTask\\mSST_v14_german_LSL.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    feedback_text = visual.TextStim(win=win, name='feedback_text',
        ...
        languageStyle='LTR',
        depth=-1.0);
    late_key_resp = keyboard.Keyboard()
    ...
    for thisBlock in blocks:
        ...
        for thisTrial_loop in trial_loop:
            ...
            late_key_resp.keys = []
            late_key_resp.rt = []
            _late_key_resp_allKeys = []
            # keep track of which components have finished
            feedback_accuracyComponents = [feedback_text, late_key_resp]
            ...
            while continueRoutine:
                ...
                # *late_key_resp* updates
                waitOnFlip = False
                
                # if late_key_resp is starting this frame...
                if late_key_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    late_key_resp.frameNStart = frameN  # exact frame index
                    late_key_resp.tStart = t  # local t and not account for scr refresh
                    late_key_resp.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(late_key_resp, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'late_key_resp.started')
                    # update status
                    late_key_resp.status = STARTED
                    # keyboard checking is just starting
                    waitOnFlip = True
                    win.callOnFlip(late_key_resp.clock.reset)  # t=0 on next screen flip
                    win.callOnFlip(late_key_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
                if late_key_resp.status == STARTED and not waitOnFlip:
                    theseKeys = late_key_resp.getKeys(keyList=['left','right'], ignoreKeys=["escape"], waitRelease=False)
                    _late_key_resp_allKeys.extend(theseKeys)
                    if len(_late_key_resp_allKeys):
                        late_key_resp.keys = _late_key_resp_allKeys[-1].name  # just the last key pressed
                        late_key_resp.rt = _late_key_resp_allKeys[-1].rt
                        late_key_resp.duration = _late_key_resp_allKeys[-1].duration
            ...
            for thisComponent in feedback_accuracyComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # check responses
            if late_key_resp.keys in ['', [], None]:  # No response was made
                late_key_resp.keys = None
            trial_loop.addData('late_key_resp.keys',late_key_resp.keys)
            if late_key_resp.keys != None:  # we had a response
                psychopy_outlet.push_sample(['late'])
                trial_loop.addData('late_key_resp.rt', late_key_resp.rt)
                trial_loop.addData('late_key_resp.duration', late_key_resp.duration)
